...
if(n > 9999) {
  ...
  uint32_t hundredths = n % 10;
  n /= 10;
  uint32_t tenths = n % 10;
  n /= 10;
  uint32_t ones = n % 10;
  n /= 10;
  uint32_t tens = n % 10;
  ...
  if(tens == 0) {
    ...
    ST7735_OutChar(tens + '0');
  }
  ST7735_OutChar(ones + '0');
  ST7735_OutChar('.');
  ST7735_OutChar(tenths + '0');
  ST7735_OutChar(hundredths + '0');
}
...
void ST7735_uBinOut6(uint32_t n){
  ...
  uint32_t tens = n % 10;
  n /= 10;
  uint32_t hundreds = n & 10;

  if(hundreds == 0) ST7735_OutChar(' '); else ST7735_OutChar(hundreds + '0');
  if(hundreds == 0 && tens == 0) ST7735_OutChar(' '); else ST7735_OutChar(tens + '0');
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]){
  int32_t displayx, displayy;
  for(int i = 0; i < num; i ++){
    if(bufX[i] < XMIN || bufX[i] > XMAX || 
      bufY[i] < YMIN || bufY[i] > YMAX) {
      continue; 
    }

    displayx = (127 * (bufX[i] - XMIN)) / (XMAX - XMIN);

    displayy = 159 - (127 * (bufY[i] - YMIN)) / (YMAX-YMIN);

    ST7735_DrawPixel((int16_t)displayx, (int16_t)displayy, ST7735_WHITE);
  }
}
