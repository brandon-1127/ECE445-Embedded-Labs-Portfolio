...
// ----------------  VARIABLES    ---------------------------------         
//
char  eid[20]           = "bmg3364_rxw62";        // TODO: Update this with you EID
char  ssid[64]          = "RolandPhone";       // TODO: Update this with your WIFI SSID
char  password[64]      = "chudmoment";   // TODO: Update this with your WIFI Password
...
const char *pub_set             = "/b2w/set"; 
const char *pub_temp            = "/b2w/temp"; 
const char *pub_fmt             = "/b2w/format";
...
void setup() {
  ...
  if (client.subscribe(topic_subscribe)){
    ...
    snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_set);  Serial.println(topic_publish);
    snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_temp); Serial.println(topic_publish);
    snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_fmt);  Serial.println(topic_publish);
    
    Serial.flush();
  #endif
 
  timer.setInterval(100L, tm4c2mqtt);             // Run the TM4C to MQTT interface every 100ms
  digitalWrite(RDY, LOW);                         // Set the RDY pin LOW
}
    ...
    if (inchar != '\n') {             // if the incoming character is not a newline then process byte the buffer position
      ...
      bufpos++;                       //  doing this over and over again until the end of package marker is read.
       ...
       if (bufpos  > 0) {
        ...
        char pf = 0; //Parse Failed
        pf |= get_next_token(hour, ser_buf, ",");
        pf |= get_next_token(minute,     NULL,    ",");
        pf |= get_next_token(second,   NULL,    ",");
        pf |= get_next_token(clk_mode,   NULL,    ",");
        pf |= get_next_token(setting_val,   NULL,    ",");
        pf |= get_next_token(temp_val,   NULL,    ",");
        pf |= get_next_token(format_val, NULL,    ",");
        ...
        snprintf(topic_publish, sizeof(topic_publish), "%s%s",  eid, pub_set); 
        client.publish(topic_publish, setting_val,1);

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_temp); 
        client.publish(topic_publish, temp_val,1);

        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_fmt); 
        client.publish(topic_publish, format_val,1);
      }
